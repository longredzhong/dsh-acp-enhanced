 ...
 *   carries torn text. Host builds that emit no `assistant/chunk` at all are
 *   covered by a fallback: the committed `assistant/message` is forwarded for
 *   whatever streaming did not already deliver, so a reply can never be lost
...
export function apply(ctx, config) {
  ...
  /**
   * Forward the committed assistant message when streaming did not deliver it.
   *
   * Streaming (`assistant/chunk` → handleChunk) is the live path, but not every
   * harness build emits those events. On such a build the only assistant
   * evidence is the `assistant/message` event, which carries the complete
   * message; without this fallback the reply never reaches the client and the
   * turn settles with usage reported and no text.
   *
   * Deduplication: whatever streaming already put on the wire this step is
   * recorded in `streamedText` / `streamedThought`, and only the remainder is
   * forwarded. When streaming delivered everything (the normal case) this is a
   * no-op, so builds that stream are unaffected. The comparison is by content,
   * not by block index, because a retry can restart a block and leave text
   * that streaming already emitted.
   */
  function emitMessageFallback(record, event) {
    ...
    const remainingText = text.slice(commonPrefixLength(text, record.streamedText))
    const remainingThought = thought.slice(commonPrefixLength(thought, record.streamedThought))
    if (remainingText.length === 0 && remainingThought.length === 0) return
    if (remainingText.length > 0) {
      record.streamedText += remainingText
      notify({
        sessionId: record.agent.session.id,
        update: {
          sessionUpdate: 'agent_message_chunk',
    ...
    if (remainingThought.length > 0) {
      record.streamedThought += remainingThought
      notify({
        sessionId: record.agent.session.id,
        update: {
          sessionUpdate: 'agent_thought_chunk',
          messageId: record.messageId,
          content: { type: 'text', text: remainingThought },
        },
      })
    }
  }
  ...
  function makeRecord(handle) {
    ...
    return {
      ...
      /** Text and reasoning the streaming path already put on the wire this
       *  step, so the `assistant/message` fallback forwards only what is
       *  missing instead of duplicating the reply. Reset per step. */
      streamedText: '',
      streamedThought: '',
